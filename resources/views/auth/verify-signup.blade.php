...
<title>FilmSpec — Verify Your Email</title>
...
.sub{font-size:12px;color:#385270;margin-bottom:20px}
...
.desc strong{color:#0060C7}
.step-badge{display:inline-flex;align-items:center;gap:6px;background:#E5F0FF;border:1px solid rgba(0,96,199,.18);border-radius:20px;padding:4px 12px;font-size:11px;color:#0060C7;font-weight:600;margin-bottom:16px}
...
<div class="card">
  ...
  <div class="sub">Account Registration</div>
  ...
  <form method="POST" action="{{ route('verify-signup.store') }}" id="otpForm" autocomplete="off">
    @csrf
    <div class="otp-row">
    ...
    <button type="submit" class="sbtn" id="verifyBtn">Confirm &amp; Create Account</button>
  </form>

  <div class="resend-row">Didn't receive it? <a href="{{ route('login') }}">Go back and try again</a></div>
